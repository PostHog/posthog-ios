...
@objc(PostHogConfig) public class PostHogConfig: NSObject {
    ...
    /// on disk, or `identify(...)` has been called, the bootstrap identity is ignored —
    /// it never overrides an already-identified user or re-links traffic across a
    /// previous anon→identified merge. Bootstrapped feature flags form a base layer only:
