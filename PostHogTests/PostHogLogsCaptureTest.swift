...
final class PostHogLogsCaptureTests {
    ...
    func captureConcurrent() async throws {
        // rate cap is disabled in setupSdk, maxBatchSize is large enough to
        // hold all records — we should see exactly 200 unique bodies on the wire.
        let sdk = setupSdk(maxBatchSize: 1000)
        defer { sdk.close() }
