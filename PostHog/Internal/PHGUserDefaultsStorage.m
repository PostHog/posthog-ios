...
@implementation PHGUserDefaultsStorage

- (instancetype)initWithDefaults:(NSUserDefaults *)defaults namespacePrefix:(NSString *)namespacePrefix crypto:(id<PHGCrypto>)crypto
{
    if (self = [super init]) {
        _defaults = defaults;
...
- (id _Nullable)plistForKey:(NSString *)key
{
    ...
    return data ? [PHGUtils plistFromData:data] : nil;
}

- (void)setPlist:(id _Nonnull)plist forKey:(NSString *)key
{
    NSData *data = [PHGUtils dataFromPlist:plist];
    if (data) {
