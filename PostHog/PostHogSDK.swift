...
@objc public class PostHogSDK: NSObject {
    ...
    ///
    /// Enable debug mode to see detailed logs about event capture, feature flag evaluation,
    /// surveys, and other internal workings of the SDK. This is useful for troubleshooting
    /// during development.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    /// ```
    ///
    /// - Parameter enabled: `true` to enable debug mode, `false` to disable. Defaults to `true`.
    ...
    ///
    /// This method returns either the ID automatically generated by PostHog or the ID
    /// that has been passed by a call to `identify()`.
    ///
    /// ## Example Usage
    /// ```swift
    /// let distinctId = PostHogSDK.shared.getDistinctId()
    /// print("Current distinct ID: \\(distinctId)")
    /// ```
    ///
    /// - Returns: The current user's distinct ID.
    ...
    ///
    /// The SDK normally flushes events automatically based on `flushAt` and `flushIntervalSeconds`
    /// configuration. Call this method to force an immediate dispatch of events.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    ///
    /// This method is typically called after a user logs out to disassociate their activity
    /// from previous sessions and prepare for a new, potentially anonymous, user journey.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    /// Registers super properties to be sent with every subsequent event.
    ///
    /// Super properties are associated with events, persisting across sessions once set.
    /// They are ideal for properties that remain constant for a user, such as `team_id` or `plan`.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    /// ```
    ///
    /// - Parameter properties: A dictionary with key-value pairs of super properties to register.
    /// - Note: This method stores properties against events, not directly against the user profile.
    ///         To store properties against the user object, use `identify(_:userProperties:userPropertiesSetOnce:)`.
    ...
    ///
    /// Super properties persist across sessions and must be explicitly removed if no longer relevant.
    /// After calling this method, subsequent events will no longer include the specified super property.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    /// ```
    ///
    /// - Parameter key: The key of the super property to unregister.
    /// - Note: To clear all super properties and other user data, use `reset()`.
    ...
    ///
    /// This is a convenience method for `identify(_:userProperties:userPropertiesSetOnce:)` without additional properties.
    /// You should call this method as soon as you are able to identify your user, typically after they log in.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    /// ```
    ///
    /// - Parameter distinctId: A unique identifier for your user in your database.
    /// - SeeAlso: [Identifying users](/docs/integrate/identifying-users)
    ...
    ///
    /// Use this method to gain insights into how your users interact with your product across sessions,
    /// devices, and platforms. All previously tracked anonymous events will be linked to this user.
    ///
    /// ## Example Usage
    /// ```swift
    /// PostHogSDK.shared.identify("user_id_from_your_database",
    ...
    ///
    /// - Parameters:
    ///   - distinctId: A unique identifier for your user in your database.
    ///   - userProperties: Optional. A dictionary with key-value pairs to set on the person profile. These are set using `$set`.
    ///   - userPropertiesSetOnce: Optional. A dictionary with key-value pairs to set on the person profile only if they don't already exist. These are set using `$set_once`.
    /// - SeeAlso: [Identifying users](/docs/integrate/identifying-users)
    /// - SeeAlso: [Person properties](/docs/product-analytics/person-properties)
    ...
    /// ```
    ///
    /// - Parameter event: The name of the event to capture. We recommend using an `[object] [verb]` format (e.g., `project created`, `user signed up`).
    ...
    ///
    /// - Parameters:
    ///   - event: The name of the event to capture. We recommend using an `[object] [verb]` format (e.g., `project created`, `user signed up`).
    ///   - properties: Optional. A dictionary with key-value pairs to include as event properties.
    ///   - userProperties: Optional. A dictionary with key-value pairs to set or update on the person profile associated with the event. These are set using `$set`.
    ...
    ///
    /// This is a convenience method for `screen(_:properties:)` without additional properties.
    ///
    /// ## Example Usage
    /// ```swift
    /// PostHogSDK.shared.screen("Settings")
    /// ```
    ///
    /// - Parameter screenTitle: The title or name of the screen being viewed.
    ...
    ///
    /// Use this method to manually track screen changes in your application, especially for SwiftUI apps
    /// where automatic screen view tracking might not provide meaningful names.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    ///
    /// - Parameters:
    ///   - screenTitle: The title or name of the screen being viewed.
    ///   - properties: Optional. A dictionary with key-value pairs to include as event properties.
    /// - Important: For SwiftUI applications, consider turning off `configuration.captureScreenViews`
    ///              and using the `.postHogScreenView()` view modifier for more meaningful screen names.
    ...
    ///
    /// Use this method when you want to associate multiple distinct IDs with a single user,
    ...
    /// ```
    ///
    /// - Parameter alias: The new distinct ID to associate with the current user.
    /// - SeeAlso: [Alias - Assigning multiple distinct IDs to the same user](/docs/data/identify#alias-assigning-multiple-distinct-ids-to-the-same-user)
    ...
    ///
    /// - Parameters:
    ///   - type: The group type (e.g., "company", "team").
    ///   - key: A unique identifier for the group in your database.
    /// - Note: Group analytics is a paid feature.
    /// - SeeAlso: [Group Analytics](/docs/user-guides/group-analytics)
    ...
    ///
    /// This method allows you to track user activity within a group while simultaneously setting
    /// or updating properties associated with that group.
    ///
    /// ## Example Usage
    /// ```swift
    ...
    ///   - type: The group type (e.g., "company", "team").
    ///   - key: A unique identifier for the group in your database.
    ///   - groupProperties: Optional. A dictionary with key-value pairs to set or update on the group profile.
    /// - Note: The `name` property is special and used in the PostHog UI. If not provided, the group ID will be used.
    /// - Note: Group analytics is a paid feature.
    /// - SeeAlso: [Group Analytics](/docs/user-guides/group-analytics)
    ...
    ///
    /// Feature flag values are cached. Call this method to refetch the latest flag values
    /// from the server if something has changed with your user (e.g., properties updated).
    ///
    /// ## Example Usage
    /// ```swift
    /// PostHogSDK.shared.reloadFeatureFlags()
    /// ```
    ///
    /// - SeeAlso: `reloadFeatureFlags(_:)` to execute code after flags are reloaded.
    /// - SeeAlso: [Feature flags](/docs/feature-flags)
    ...
    ///
    /// Use this method to refetch the latest flag values and perform actions immediately
    /// after the flags have been successfully reloaded.
    ///
    /// ## Example Usage
    /// ```swift
    /// PostHogSDK.shared.reloadFeatureFlags {
    ...
    /// ```
    ///
    /// - Parameter callback: A closure to be executed after feature flags are reloaded.
    /// - SeeAlso: [Feature flags](/docs/feature-flags)
    ...
    ///
    /// This method is used to get the value of both boolean and multivariate feature flags.
    ///
    /// ## Example Usage
    /// ```swift
    /// if (PostHogSDK.shared.getFeatureFlag("flag-key") as? String == "variant-key") {
    ///     // Do something differently for this user based on the variant
    /// }
    /// ```
    ///
    /// - Parameter key: The key of the feature flag.
    /// - Returns: The value of the feature flag (e.g., `Bool`, `String`), or `nil` if the flag is not found.
    /// - SeeAlso: [Feature flags](/docs/feature-flags)
    ...
    ///
    /// This is typically used with multivariate feature flags that deliver a specific value (the payload).
    ///
    /// ## Example Usage
    /// ```swift
    ...
    ///
    /// - Parameter key: The key of the feature flag.
    /// - Returns: The payload of the feature flag, or `nil` if no payload is associated or the flag is not found.
    /// - SeeAlso: [Feature flags](/docs/feature-flags)
    ...
    ///     print("User is opted out.")
    /// } else {
    ///     print("User is opted in.")
    /// }
    /// ```
    ///
    /// - Returns: `true` if the user is opted out, `false` otherwise.
