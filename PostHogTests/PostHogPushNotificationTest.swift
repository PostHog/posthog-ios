    ...
    // Regression for the opt-out / unregister race found during the RN push review (posthog-ios#746):
    // opt-out during an in-flight unregister must still send the DELETE. Lives in this in-target file
    // (not a standalone one) so `test-ios-simulator` actually compiles and runs it.
