...
class FileStorageTest : QuickSpec {
  override func spec() {
    ...
    it("Creates caches directory") {
      let url = PHGFileStorage.cachesDirectoryURL()
      expect(url).toNot(beNil())
      expect(url?.pathComponents[url!.pathComponents.count - 2]) == "Caches"
      expect(url?.lastPathComponent) == Bundle.main.bundleIdentifier
    }
    ...
    it("creates folder if none exists") {
      ...
      _ = PHGFileStorage(folder: url!, crypto: nil)
