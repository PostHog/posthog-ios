    ...
    struct PostHogReplayCaptureTouchesTests {
        ...
        @Test("Touch capture can be disabled and re-enabled at runtime from a background thread")
        func runtimeToggle() async throws {
            let (sut, integration, snapshots) = try makeSut()
            defer { sut.close() }
            let config = sut.config.sessionReplayConfig
            let window = UIWindow()
            integration.handleApplicationEvent(event: TouchEvent(), date: Date(), window: window)
            await drainReplayQueue()
            #expect(snapshots.touches.count == 2)
            await Task.detached { config.captureTouches = false }.value
            let disabledEvent = TouchEvent()
            integration.handleApplicationEvent(event: disabledEvent, date: Date(), window: window)
            await drainReplayQueue()
            #expect(!disabledEvent.touchesRead)
            #expect(snapshots.touches.count == 2)
            #expect(sut.isSessionReplayActive())
            await Task.detached { config.captureTouches = true }.value
            integration.handleApplicationEvent(event: TouchEvent(), date: Date(), window: window)
            await drainReplayQueue()
            #expect(snapshots.touches.count == 4)
        }

        @Test("The SDK setter fails before setup and after close")
        func setterRequiresSetup() {
            let sut = PostHogSDK.with(PostHogConfig(projectToken: ""))
            #expect(!sut.setCaptureTouches(false))
            #expect(sut.config.sessionReplayConfig.captureTouches)
            sut.close()
            #expect(!sut.setCaptureTouches(true))
        }

        @Test("The SDK setter updates the installed config even after an ignored second setup")
        func setterUsesInstalledConfig() async throws {
            let (sut, integration, snapshots) = try makeSut()
            defer { sut.close() }
            let installedConfig = sut.config.sessionReplayConfig
            let ignoredConfig = PostHogConfig(projectToken: UUID().uuidString)
            sut.setup(ignoredConfig)
            #expect(sut.config.sessionReplayConfig === installedConfig)
            let disabled = await Task.detached { sut.setCaptureTouches(false) }.value
            #expect(disabled)
            #expect(!installedConfig.captureTouches)
            #expect(ignoredConfig.sessionReplayConfig.captureTouches)
            integration.handleApplicationEvent(event: TouchEvent(), date: Date(), window: UIWindow())
            await drainReplayQueue()
            #expect(snapshots.touches.isEmpty)
            #expect(sut.setCaptureTouches(true))
            #expect(installedConfig.captureTouches)
        ...
        @Test("Disabling drops touch work already waiting on the replay queue")
        func queuedTouchesSuppressed() async throws {
            let (sut, integration, snapshots) = try makeSut()
            defer { sut.close() }
            // Suspend deterministically before submitting, rather than racing a timed delay.
            PostHogReplayIntegration.dispatchQueue.suspend()
            let event = TouchEvent()
            integration.handleApplicationEvent(event: event, date: Date(), window: UIWindow())
            #expect(event.touchesRead)
            sut.config.sessionReplayConfig.captureTouches = false
            PostHogReplayIntegration.dispatchQueue.resume()
            await drainReplayQueue()
            #expect(snapshots.touches.isEmpty)
        }
