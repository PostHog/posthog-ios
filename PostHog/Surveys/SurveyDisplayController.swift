...
//
    ...
    final class SurveyDisplayController: ObservableObject {
        ...
        func onNextQuestion(index _: Int, response: PostHogSurveyResponse) {
            guard let displayedSurvey else { return }
            guard let next = onSurveyResponse?(displayedSurvey, currentQuestionIndex, response) else { return }
