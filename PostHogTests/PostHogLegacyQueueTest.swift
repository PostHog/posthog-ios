...
@Suite("PostHogLegacyQueue Tests")
struct PostHogLegacyQueueTest {
    func getBaseUrl() -> URL {
        let tempDir = FileManager.default.temporaryDirectory
        return tempDir.appendingPathComponent(UUID().uuidString)
    }

    @Test("migrate old queue to new queue")
    func migrateOldQueueToNewQueue() throws {
    ...
    @Test("ignore and delete corrupted file")
    func ignoreAndDeleteCorruptedFile() throws {
        let baseUrl = getBaseUrl()
        try FileManager.default.createDirectory(atPath: baseUrl.path, withIntermediateDirectories: true)
