...
@implementation NSData (POSTHOG_GZIP)

- (NSData *)posthog_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self posthog_isGzippedData]) {
        return self;
    }

    void *libz = posthog_libzOpen();
...
- (NSData *)posthog_gzippedData
{
    return [self posthog_gzippedDataWithCompressionLevel:-1.0f];
}

- (BOOL)posthog_isGzippedData
{
